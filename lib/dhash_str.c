...
static uint32_t Murmur3_32(const char* key);
...
static void dhash_str_do_insert(dhash_str_t* dhash, const char* key, dhash_info_t info)
{
    int num_buckets = prime_set[dhash->num_buckets_idx];
    uint32_t hash = Murmur3_32(key) % num_buckets;
...
void dhash_str_insert(dhash_str_t* dhash, const char* key, dhash_info_t info)
{
    if (key == NULL) abort();
    if (info == NULL) abort();
...
void dhash_str_walk(dhash_str_t* dhash, dhash_walk_fn walk_fn, void *walk_info)
{
    int num_buckets = prime_set[dhash->num_buckets_idx];
