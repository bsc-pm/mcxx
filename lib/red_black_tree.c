...
struct rb_red_blk_node_tag {
  const void* key;
  void* info;
  int red; /* if red=0 then the node is black */
  struct rb_red_blk_node_tag* left;
  struct rb_red_blk_node_tag* right;
  struct rb_red_blk_node_tag* parent;
};


/* comp_func(a,b) should return 1 if *a > *b, -1 if *a < *b, and 0 otherwise */
/* Destroy(a) takes a pointer to whatever key might be and frees it accordingly */
struct rb_red_blk_tree_tag {
  int (*comp_func)(const void* a, const void* b); 
  void (*key_dtor_func)(const void* a);
  void (*info_dtor_func)(const void* a);
...
/*  "greater than" the second.   key_dtor_func takes a pointer to a key and */
/*  destroys it in the appropriate manner when the node containing that */
/*  key is deleted.  InfoDestFunc is similiar to key_dtor_func except it */
...
static rb_red_blk_node* rb_tree_successor(rb_red_blk_tree* tree,rb_red_blk_node* x) { 
  rb_red_blk_node* y;
  rb_red_blk_node* nil=tree->nil;
...
#if 0
static rb_red_blk_node* rb_tree_predecessor(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  rb_red_blk_node* y;
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* root=tree->root;
...
rb_red_blk_node* rb_tree_query(rb_red_blk_tree* tree, const void* q) {
...
/*             and <= high with respect to the comp_func function */
/**/
...
const void* rb_node_get_key(rb_red_blk_node* n)
{
    return n->key;
}

void* rb_node_get_info(rb_red_blk_node* n)
{
    return n->info;
}
