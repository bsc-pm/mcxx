...
double dot_product_1(double A[N], double B[N])
{
    ...
    for (i=0; i<N; i+=CHUNK_SIZE) {
        ...
        #pragma analysis_check assert correctness_auto_storage(A, B, C) \
                                      correctness_pointer_deps(A, B) correctness_race(C[j]) \
                                      correctness_incoherent_in(C) correctness_incoherent_out(A, B)
...
double dot_product_2(double A[N], double B[N])
{
    ...
    for (i=0; i<N; i+=CHUNK_SIZE) {
        ...
        #pragma analysis_check assert correctness_pointer_deps(C) correctness_race(C[j]) \
