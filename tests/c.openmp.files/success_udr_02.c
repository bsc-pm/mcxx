...
#pragma omp declare reduction type(my_complex_t) operator(complex_add, complex_sub) identity({0,0}) order(right)

#pragma omp declare reduction type(my_complex_t) operator(complex_mul) identity({1,0}) order(right)
