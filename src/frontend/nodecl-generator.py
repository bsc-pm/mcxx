...
def generate_visitor_class_header(rule_map):
    ...
    print "{"
    print "   public:"
    print "     void walk(const NodeclBase&); /* If you override this member function you will be fired */"
    for class_name in classes:
        print "     virtual void visit(const Nodecl::%s &) = 0;" % (class_name)
...
def generate_visitor_class_impl(rule_map):
    ...
    print ""
    print "namespace Nodecl {"
    print "void BaseNodeclVisitor::walk(const NodeclBase& n)"
    print "{"
    print """
    if (n.is_null())
        return;
    ...
    {
        case AST_NODE_LIST: { AST tree = nodecl_get_ast(n._n); AST it; for_each_element(tree, it) { AST elem = ASTSon1(it); NodeclBase nb(::_nodecl_wrap(elem)); this->walk(nb); } break; }
"""
    node_kind = set([])
    for rule_name in rule_map:
        rule_rhs = rule_map[rule_name]
...
def generate_nodecl_classes_base(rule_map):
   ...
   print "    friend class BaseNodeclVisitor;"
   print "};"
