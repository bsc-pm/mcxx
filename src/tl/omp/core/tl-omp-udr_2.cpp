...
namespace TL
{
    namespace OpenMP
    {
        struct udr_valid_prototypes_t
        {
            ...
            Type first_arg;
            Type second_arg;
            // OpenMP::UDRInfoItem2::Associativity default_assoc;
            bool allows_left;
            bool allows_right;
        };
        ...
        {
			Type canonic_reduction_type = _reduction_type;
            canonic_reduction_type = canonic_reduction_type.get_canonical_type();

			std::stringstream ss;
			ss << canonic_reduction_type.get_internal_type();

            if (_is_builtin)
            {
                return ".udr_" + _builtin_op + "_" + ss.str();
            }
            else if (is_operator_tree(_op_expr.get_ast()))
            {
                return ".udr_" + _op_expr.prettyprint() + "_" + ss.str();
            }
            ...
            {
                return ".udr_" + _op_expr.get_unqualified_part() + "_" + ss.str();
            }
        }
                ...
                const std::string& filename, int line) const
        {
            ...
            ObjectList<UDRInfoItem2> udr_lookup;
            {
            ...
            {
                UDRInfoItem2 result;
                for (ObjectList<UDRInfoItem2>::iterator it = udr_lookup.begin();
            ...
            // Only C++ here
            ObjectList<UDRInfoItem2> templated_udrs;
            ObjectList<UDRInfoItem2> viable_udr;
            ...
            if (!templated_udrs.empty())
            {
                ...
                for (ObjectList<UDRInfoItem2>::iterator it = templated_udrs.begin();
            ...
            ObjectList<Symbol> tentative_result;
            for (ObjectList<UDRInfoItem2>::iterator it = viable_udr.begin();
        ...
        Symbol UDRInfoItem2::get_deduction_function() 
        {
            if (!_is_template)
                return Symbol(NULL);
