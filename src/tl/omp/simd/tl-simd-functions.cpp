...
/*
   ...
   -- SIMD: 
        Function annotated with #pragma omp simd
   -- ARCH_DEFAULT: 
...
// Global structure for SIMD functions
TL::SIMD::GenericFunctions generic_functions;
...
void ReplaceSrcGenericFunction::add_replacement(TL::Symbol sym, const std::string& str)
{
...
void GenericFunctions::add_naive_generic_function(const TL::Symbol& scalar_func_sym)
{
    ERROR_CONDITION(!scalar_func_sym.is_valid(), "'scalar_func_sym' is invalid.", 0);

    DEBUG_CODE()
    {
        std::cerr << "SIMD: Adding NAIVE generic function '" 
            << scalar_func_sym.get_name()
            << "'"
            << std::endl;
    }

    // Function is inserted only once because 
    // if GenericFunctionInfo exists, NAIVE is implicit.
    function_map_t::iterator it =  _function_map.find(scalar_func_sym);

    if (it == _function_map.end())
    {
        _function_map.insert(std::make_pair(
                    scalar_func_sym, GenericFunctionInfo(scalar_func_sym)));
    }
    else
    {
        DEBUG_CODE()
        {
            std::cerr << "SIMD: Ignoring NAIVE generic function for scalar function '"
                << scalar_func_sym.get_name()
                << "'. Another generic function was inserted before." 
                << std::endl;
        }
    }
}
