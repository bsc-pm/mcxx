...
namespace {
    bool is_accepted_induction_variable_syntax(Node* loop, NBase stmt,
                                                NBase& iv, NBase& incr)
    {
        bool is_iv = false;

        Optimizations::ReduceExpressionVisitor v;
        NBase st = stmt.shallow_copy();
        v.walk(st);
        ...
        {
            NBase rhs = st.as<Nodecl::Preincrement>().get_rhs();
            iv = rhs;
        ...
        {
            NBase rhs = st.as<Nodecl::Postincrement>().get_rhs();
            iv = rhs;
            incr = Nodecl::IntegerLiteral::make(rhs.get_type(), const_value_get_one(/* bytes */ 4, /* signed */ 1));
            is_iv = true;
        }
        ...
        {
            NBase rhs = st.as<Nodecl::Predecrement>().get_rhs();
            iv = rhs;
        ...
        {
            NBase rhs = st.as<Nodecl::Postdecrement>().get_rhs();
            iv = rhs;
            incr = Nodecl::IntegerLiteral::make(rhs.get_type(), const_value_get_minus_one(/* bytes */ 4, /* signed */ 1));
            is_iv = true;
        }
