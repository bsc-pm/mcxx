...
namespace Utils {
    ...
    bool ext_sym_set_contains_enclosed_nodecl( ExtendedSymbol ei, ext_sym_set sym_set )
    {
        ...
        for( ext_sym_set::iterator it = sym_set.begin( ); it != sym_set.end( ); ++it )
        {
            if( ext_sym_set_contains_enclosing_nodecl( *it, fake_set ) )
                return true;
        }
        return false;
    }
    ...
    void delete_enclosing_var_from_list( ExtendedSymbol ei, ext_sym_set& sym_set )
    {
        for( ext_sym_set::iterator it = sym_set.begin( ); it != sym_set.end( ); ++it)
        {
            ext_sym_set fake_set;
            fake_set.insert( *it );
            if( ext_sym_set_contains_enclosing_nodecl( ei, fake_set ) )
            {
                sym_set.erase( it );
                return;
            }
    ...
    void delete_enclosed_var_from_list( ExtendedSymbol ei, ext_sym_set& sym_set )
    {
        for( ext_sym_set::iterator it = sym_set.begin( ); it != sym_set.end( ); ++it )
        {
            ext_sym_set fake_set;
            fake_set.insert( *it );
            if( ext_sym_set_contains_enclosed_nodecl( ei, fake_set ) )
            {
                sym_set.erase( it );
                return;
            }
