 ...
 */
            ...
            int &next_domain_id)
    {
        ...
        {
            AliveTaskSet tmp_alive_tasks_of_current = current->get_live_in_tasks();
            ...
            if (tmp_alive_tasks_of_current != current->get_live_in_tasks())
            {
                current->get_live_in_tasks() = tmp_alive_tasks_of_current;
        ...
        std::cerr << "["
            ...
            << "]"
            << "Before: IN(" << current->get_id() << ") = " << print_set(current->get_live_in_tasks()) << " "
            << "OUT(" << current->get_id() << ") = " << print_set(current->get_live_out_tasks()) << std::endl;
                    ...
                    case TaskSync_Yes : // Note that we do something slightly different below
                        {
                            ...
                            {
                                size_t removed = current->get_live_out_tasks().erase(*alive_tasks_it);
                                if (removed != 0)
                                {
                                    std::cerr << "CHANGED " << __FILE__ << ":" << __LINE__
        ...
        {
            if (current->get_live_in_tasks() != current->get_graph_entry_node()->get_live_in_tasks())
            {
                current->get_graph_entry_node()->get_live_in_tasks() = current->get_live_in_tasks();
            ...
            if (current->get_live_out_tasks() != current->get_graph_exit_node()->get_live_out_tasks())
            {
                current->get_live_out_tasks() = current->get_graph_exit_node()->get_live_out_tasks();
        ...
        else if (current->is_omp_taskwait_node())
        {
            ...
            if (still_alive != current->get_live_out_tasks())
            {
        ...
        else if (current->is_omp_barrier_node())
        {
            ...
            if (!current->get_live_out_tasks().empty())
            {
        ...
        std::cerr << "["
            ...
            << ":" << (current->is_graph_node() ? current->get_graph_type_as_string() : "")
            << "]"
            << "After : IN(" << current->get_id() << ") = " << print_set(current->get_live_in_tasks()) << " "
            << "OUT(" << current->get_id() << ") = " << print_set(current->get_live_in_tasks()) << std::endl;
