...
namespace Analysis {
    ...
    SuitableAlignmentVisitor::SuitableAlignmentVisitor(const Utils::InductionVarList induction_variables,
    ...
    int SuitableAlignmentVisitor::visit(const Nodecl::Symbol& n)
    {
        ...
        {
            Utils::InductionVar* iv = Utils::get_induction_variable_from_list(_induction_variables, n);
            Optimizations::ReduceExpressionVisitor v;
    ...
    ExpressionEvolutionVisitor::ExpressionEvolutionVisitor(ObjectList<Analysis::Utils::InductionVar*> ivs,
                                                           const NodeclSet& killed, Node* scope, Node* n_node, ExtensibleGraph* pcfg)
    ...
    bool ExpressionEvolutionVisitor::variable_is_iv(const Nodecl::NodeclBase& n)
    {
        ...
        {
            if(Nodecl::Utils::structurally_equal_nodecls((*it)->get_variable(), n, /* skip conversion nodes */ true))
            {
                _ivs.insert(*it);
                is_iv = true;
                break;
    ...
    bool ExpressionEvolutionVisitor::node_uses_iv(Node* node)
    {
        ...
        {
            if(node->uses_var((*it)->get_variable()))
                result = true;
        }
        return result;
    }
    ...
    bool ExpressionEvolutionVisitor::definition_depends_on_iv(const Nodecl::NodeclBase& n, Node* node)
    {
        ...
        {   // Check whether the expression used to modify it depends on an induction variable
            result = Nodecl::Utils::stmtexpr_contains_nodecl_structurally(n, (*it)->get_variable());
        }
        if(!result)
        {
            NodeclMap reaching_defs_in = node->get_reaching_definitions_in();
            for(NodeclMap::iterator it = reaching_defs_in.begin(); it != reaching_defs_in.end() && !result; ++it)
            {
                if(Nodecl::Utils::stmtexpr_contains_nodecl_structurally(it->first, n))
            ...
            if(visit_node)
            {
                ...
                if(killed.find(n) != killed.end())
                {
                    ...
                    {   // Case 2: This checks situations such as:
                        ...
                        NodeclMap reaching_defs_out = current->get_reaching_definitions_out();
                        for (NodeclMap::iterator it = reaching_defs_out.begin();
    ...
    bool ExpressionEvolutionVisitor::visit(const Nodecl::ArraySubscript& n)
    {
        ...
        return !Utils::nodecl_set_contains_nodecl(n, _killed);
    }
    ...
    bool ExpressionEvolutionVisitor::visit(const Nodecl::Symbol& n)
    {
        ...
        {
            NodeclMap reach_def_in = _n_node->get_reaching_definitions_in();
            //         NodeclMap reach_def_out = _n_node->get_reaching_definitions_out();
            ...
            if(reach_def_in.find(n)!=reach_def_in.end())
            {
                std::pair<NodeclMap::iterator, NodeclMap::iterator> def_nodes = reach_def_in.equal_range(n);
                for(NodeclMap::iterator it = def_nodes.first; it != def_nodes.second; ++it)
                {
        ...
        Utils::InductionVar* iv = NodeclStaticInfo::get_nested_induction_variable(_scope_node, _n_node, n);
        if(iv!=NULL)
        {
            _has_constant_evolution = !Utils::nodecl_set_contains_nodecl(n, _killed);
        ...
        return !Utils::nodecl_set_contains_nodecl(n, _killed) || !var_is_iv_dependent_in_scope(n);
    }
