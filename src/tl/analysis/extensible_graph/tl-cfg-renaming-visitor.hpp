...
namespace TL
{
    ...
    class LIBTL_CLASS CfgRenamingVisitor : public Nodecl::NodeclVisitor<Nodecl::NodeclBase>
    {
        private:
            ...
            int _line;
            int _i;     // id for the new 
            
            
            // *** Auxiliar methods *** //
            template <typename T>
            scope_entry_t* create_symbol(const T& n, Nodecl::NodeclBase init_expr);
            
            nodecl_t create_nodecl_list(ObjectList<Nodecl::NodeclBase> list);
            
            template <typename T>
            Ret empty_visit(const T& n);
        ...
        public:
            // *** Constructors *** //
            CfgRenamingVisitor(std::map<Symbol, Nodecl::NodeclBase> rename_map, const char* _filename, int _line, int i);
