...
namespace TL
{
    namespace Analysis
    {
        ...
        static ObjectList<Node*> uses_from_node_to_node(Node* current, Node* end, ExtendedSymbol ei, Node* task);
        ...
        static void purge_set_of_auto_scoped_array(ExtendedSymbol ei, ext_sym_set list)
        {
        ...
        void StaticAnalysis::mix_array_computations(Node* task_node)
        {
            ...
            for(ext_sym_set::iterator it = _undefined_vars.begin(); it != _undefined_vars.end(); ++it)
            {
                ...
                new_undef.insert(ExtendedSymbol(ExtendedSymbol::get_nodecl_base(it->get_nodecl())));
                if (it->is_array())
                {
            ...
            {
                // Add the whole array to the new set
                new_shared.insert(ExtendedSymbol(ExtendedSymbol::get_nodecl_base(it->get_nodecl())));
                // Delete it from the other groups
                if (it->is_array())
                {    
                    purge_set_of_auto_scoped_array(*it, _firstprivate_vars);
            ...
            {
                // Add the whole array to the new set
                new_firstprivate.insert(ExtendedSymbol(ExtendedSymbol::get_nodecl_base(it->get_nodecl())));
                // Delete it from the other groups
        ...
        */
        ...
        void StaticAnalysis::scope_variable(Node* task, Node* ei_node, char usage, ExtendedSymbol ei, bool is_in_loop, ext_sym_set& scoped_vars)
        {
...
//                 ExtensibleGraph::clear_visits_aux(task);
        ...
        bool StaticAnalysis::task_and_simultaneous_only_read(Node* task, ExtendedSymbol ei)
        {
            ext_sym_set task_killed = task->get_killed_vars();
        ...
        bool StaticAnalysis::task_reads_and_writes(Node* task, ExtendedSymbol ei)
        {
        ...
        void StaticAnalysis::task_reads_and_writes_rec(Node* task, Node* current, ExtendedSymbol ei, bool& read, bool& write)
        {
        ...
        bool StaticAnalysis::scope_ie_in_iterated_task(Node* task, Node* current, Node* ei_node, char usage, ExtendedSymbol ei)
        {
            if (!current->is_visited_aux())
            {
                current->set_visited_aux(true);
        ...
        ObjectList<Node*> StaticAnalysis::var_uses_out_task(Node* task, ExtendedSymbol ei)
        {
        ...
        ObjectList<Node*> StaticAnalysis::var_uses_in_task(Node* current, ExtendedSymbol ei)
        {
            ObjectList<Node*> uses;
        ...
        bool StaticAnalysis::race_condition(Node* task, ExtendedSymbol ei)
        {
            bool res = false;
        ...
        static bool nodecl_is_base_of_symbol(Symbol s, Nodecl::NodeclBase n)
        {
            ...
            else
            {
                ...
                {
                    Nodecl::Dereference n_ = n.as<Nodecl::Dereference>();
         ...
         * \param n   Nodecl being used/defined which has been stored in an ExtendedSymbol                  -> a.b.c
        ...
        static Nodecl::NodeclBase match_nodecl_with_symbol(Nodecl::NodeclBase n, Symbol s, Nodecl::NodeclBase arg)
        {
            if (!nodecl_is_constant_value(arg))
            {
                ...
                else if (n.is<Nodecl::Symbol>() || n.is<Nodecl::PointerToMember>())
                {
                    if (n.get_symbol() == s)
                    {
                        return Nodecl::Utils::deep_copy(arg, arg);
                    }
                }
                else if (n.is<Nodecl::ClassMemberAccess>())
                {
                ...
                else if (n.is<Nodecl::Dereference>())
                {
                    ...
                    if (!rhs.is_null())
                    {
                        return Nodecl::Dereference::make(rhs, arg_type, arg_file, arg_line);
                    }
                }
        ...
        */
        ...
        static void set_usage_to_symbol_in_node(Node* node, struct var_usage_t* n, ExtensibleGraph* called_func_graph)
        {
            ...
            if (usage == '0')
            {
                ExtendedSymbol ue_var(n->get_nodecl());
                node->set_ue_var(ue_var);
            }
            else if (usage == '1')
            {
                ExtendedSymbol killed_var(n->get_nodecl());
                node->set_killed_var(killed_var);
            }
            else if (usage == '2')
            {
                ExtendedSymbol ue_killed_var(n->get_nodecl());
        ...
        bool PCFGVisitor::func_has_cyclic_calls_rec(Symbol reach_func, Symbol stop_func, ExtensibleGraph * graph)
        {
            ObjectList<Symbol> called_syms = graph->get_function_calls();
           
            ObjectList<ExtensibleGraph*> pcfgs = TL::Analysis::AnalysisSingleton::get_analysis()->get_pcfgs();
            for (ObjectList<Symbol>::iterator it = called_syms.begin(); it != called_syms.end(); ++it)
            {
                ExtensibleGraph* called_func_graph = find_function_for_ipa(*it, pcfgs);
        ...
        bool PCFGVisitor::func_has_cyclic_calls(Symbol reach_func, ExtensibleGraph * graph)
        {
            return func_has_cyclic_calls_rec(reach_func, graph->get_function_symbol(), graph);
        }

        void PCFGVisitor::set_live_initial_information(Node* node)
        {
            if (node->get_type() == BASIC_FUNCTION_CALL_NODE)
            {
                ObjectList<ExtensibleGraph*> pcfgs = TL::Analysis::AnalysisSingleton::get_analysis()->get_pcfgs();
                ExtensibleGraph* called_func_graph = find_function_for_ipa(node->get_function_node_symbol(), pcfgs);
                ...
                if (called_func_graph != NULL)
                {
                    ...
                    {   // Recursive analysis
                        ...
                        if (!glob_vars.empty() || !ref_params_to_args.empty())
                        {
                            ...
                            CfgIPAVisitor ipa_visitor(called_func_graph, pcfgs, glob_vars, ref_params, ref_params_to_args);
                            ...
                            for (ObjectList<struct var_usage_t*>::iterator it = ipa_usage.begin(); it != ipa_usage.end(); ++it)
                            {
                                ...
                                if (usage == '0')
                                {
                                    killed_vars.insert(ExtendedSymbol(s));
                                }
                                else if (usage == '1')
                                {
                                    ue_vars.insert(ExtendedSymbol(s));
                                }
                                else if (usage == '2')
                                {
                                    killed_vars.insert(ExtendedSymbol(s));
                                ...
                                {
                                    undef_vars.insert(ExtendedSymbol(s));
                                }
                                else
                                {
                            ...
                            for (std::map<Symbol, Nodecl::NodeclBase>::iterator it = ref_params_to_args.begin(); it != ref_params_to_args.end(); ++it)
                            {
                                ...
                                if (!s.is<Nodecl::FunctionCall>() && !s.is<Nodecl::VirtualFunctionCall>())
                                {
                                    ...
                                    if (s.is<Nodecl::Symbol>() || s.is<Nodecl::Cast>() || s.is<Nodecl::ClassMemberAccess>()
                                        || s.is<Nodecl::Reference>() || s.is<Nodecl::Dereference>() 
                                    ...
                                    for (ObjectList<Nodecl::NodeclBase>::iterator its = arg_ei.begin(); its != arg_ei.end(); ++its)
                                    {
                                        ...
                                        {   // All arguments that are not already in the list, have to be set as UE
                                            ue_vars.insert(ExtendedSymbol(*its));
                                        }
                                        else
                                        {   // If some list contains a part of the argument, then the argument have an undefined 
                                                ...
                                                && ext_sym_set_contains_nodecl(*its, killed_vars))
                                            {
                                                ...
                                                undef_vars.insert(ExtendedSymbol(*its));
                                            }
                        ...
                        for (ObjectList<Nodecl::NodeclBase>::iterator it = non_ref_args.begin(); it != non_ref_args.end(); ++it)
                        {
                            if (it->is<Nodecl::Symbol>() || it->is<Nodecl::Cast>() || it->is<Nodecl::ClassMemberAccess>()
                                || it->is<Nodecl::Reference>() || it->is<Nodecl::Dereference>() 
                                || it->is<Nodecl::ArraySubscript>()) 
                            {
                                ue_vars.insert(ExtendedSymbol(*it));
                            }
                    ...
                    else
                    {
                        if (!_visited_functions.contains(called_func_graph->get_function_symbol()))
                        {
                            ...
                            {
                                ExtensibleGraph* current_pcfg = _current_pcfg;
                                _current_pcfg = called_func_graph;
                               
                                compute_use_def_chains(called_func_graph->get_graph());
                                if (called_func_graph->has_use_def_computed() == '2')
                                    _current_pcfg->set_use_def_computed('2');
                                else
                                    called_func_graph->set_use_def_computed('1');
                                
                                _current_pcfg = current_pcfg;
                            }
                            ...
                            for(ext_sym_set::iterator it = called_func_ue_vars.begin(); it != called_func_ue_vars.end(); ++it)
                            {
                                ...
                                {   // UE variable is a parameter or a part of a parameter
                                    if (ue_arg.is<Nodecl::Symbol>() || ue_arg.is<Nodecl::Cast>() || ue_arg.is<Nodecl::ClassMemberAccess>()
                                        || ue_arg.is<Nodecl::Reference>() || ue_arg.is<Nodecl::Dereference>() 
                                        || ue_arg.is<Nodecl::ArraySubscript>()) 
                                    {
                                        ExtendedSymbol ei(ue_arg);
                            ...
                            for (ObjectList<Nodecl::NodeclBase>::iterator it = non_ref_args.begin(); it != non_ref_args.end(); ++it)
                            {
                                if (it->is<Nodecl::Symbol>() || it->is<Nodecl::Cast>() || it->is<Nodecl::ClassMemberAccess>()
                                    ...
                                    || it->is<Nodecl::ArraySubscript>()) 
                                {
                                    node_ue_vars.insert(ExtendedSymbol(*it));
                                }
                                else if (it->is<Nodecl::FunctionCall>() || it->is<Nodecl::VirtualFunctionCall>())
                                {}  // Nothing to do, we don't need to propagate the usage of a temporal value
                                else
                                {
                                    ...
                                    for (ObjectList<Nodecl::NodeclBase>::iterator its = nodecl_symbols.begin(); 
                                            its != nodecl_symbols.end(); ++its)
                                    {
                                        node_ue_vars.insert(ExtendedSymbol(*its));
                                    }
                                }
                            }
                ...
                else
                {
                    ...
                    ObjectList<struct var_usage_t*> current_global_vars = _current_pcfg->get_global_variables();
        ...
        static void compute_params_usage_in_unknown_func_call(Nodecl::NodeclBase n, ext_sym_set& undef_behaviour_vars, ext_sym_set& ue_vars)
        {
            ...
            {
                ue_vars.insert(ExtendedSymbol(n));
            }
            else if (n.is<Nodecl::Reference>() || n.is<Nodecl::Dereference>())
            {
                ExtendedSymbol ei(n);
        ...
        void PCFGVisitor::gather_live_initial_information(Node* node)
        {
            if (!node->is_visited())
            {
                node->set_visited(true);
        ...
        void PCFGVisitor::compute_use_def_chains(Node* node)
        {
            if (CURRENT_CONFIGURATION->debug_options.analysis_verbose)
                std::cerr << "  ==> Graph '" << _current_pcfg->get_name() << "'" << std::endl;
            gather_live_initial_information(node);
            ExtensibleGraph::clear_visits(node);
            _visited_functions.clear();
        }
        
        void PCFGVisitor::analyse_induction_variables(ExtensibleGraph* graph)
        {
            Node* node = graph->_graph;
            if (CURRENT_CONFIGURATION->debug_options.analysis_verbose)
                std::cerr << "  ==> Graph '" << _current_pcfg->get_name() << "'" << std::endl;
            ObjectList<ExtensibleGraph*> pcfgs = TL::Analysis::AnalysisSingleton::get_analysis()->get_pcfgs();
            LoopAnalysis loop_analysis(pcfgs);
        ...
        void PCFGVisitor::analyse_loops(Node* node)
        {
            ObjectList<ExtensibleGraph*> pcfgs = TL::Analysis::AnalysisSingleton::get_analysis()->get_pcfgs();
            LoopAnalysis loop_analysis(pcfgs);
