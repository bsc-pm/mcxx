...
namespace TL
{
    ...
    void OpenMPTransform::run(DTO& dto)
    {
                ...
                it++)
        {
            ...
            TL::AST_t function_name_tree = enclosing_function_def.get_attribute("a");
            std::string function_name_str = function_name_tree.prettyprint();

            TL::AST_t parallel_body = it->get_attribute("b");

            TL::Source outlined_function;
            outlined_function << "void outline_" << "prova" << "(";
