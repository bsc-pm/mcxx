...
namespace TL { namespace Nanos6 {
    ...
    void EnvironmentCapture::add_storage_for_standalone_field(const std::string &name, TL::Type type)
    {
        TL::Type type_of_field = type.no_ref();

        if (type_of_field.is_function())
        {
            if (IS_FORTRAN_LANGUAGE)
            {
                type_of_field = TL::Type::get_void_type().get_pointer_to();
        ...
        _standalone_field_map[name] = std::pair<TL::Symbol, TL::Type>(field, type);
    ...
    EnvironmentCapture::Accessor EnvironmentCapture::get_standalone_field_accessor(
        const TL::Symbol& object,
        ...
        bool actual_storage_if_vla) const
    {
        ...
        if (actual_storage_if_vla &&
            (IS_C_LANGUAGE || IS_CXX_LANGUAGE) &&
            (type.depends_on_nonconstant_values() &&
                (type.no_ref().is_array() ||
                type.no_ref().is_pointer())))
        {
            // A conversion between pointer type (from argument) and
            // array type (from parameter) is required. This is done by
            // getting a reference (&) from the argument, casting it to a
            ...
            TL::Type param_type = rewrite_type_using_args(
                object, type.no_ref(), TL::ObjectList<TL::Symbol>(), TL::ObjectList<TL::Symbol>());
        ...
        Accessor result = {
            /* _original_symbol */ TL::Symbol::invalid(),
            /* _original_type */ type,
            /* _environment_symbol */ field,
            /* _environment_name */ field.get_name(),
            /* _environment_type */ field.get_type().get_lvalue_reference_to(),
    ...
    Nodecl::List EnvironmentCapture::emit_standalone_field_allocation(
        TL::Scope context,
        const TL::Symbol& destination_environment,
        const std::string& field_name,
        /* inout */
        Nodecl::NodeclBase &vla_offset)
    {
        ...
        // If the privatized symbol is neither a VLA nor a symbol that is
        // represented as an allocatable variable in the environment structure, skip it!
        if (!destination_accessor._original_type.depends_on_nonconstant_values()
                && !destination_accessor._environment_symbol.is_allocatable())
            return result;
        ...
        if (IS_C_LANGUAGE || IS_CXX_LANGUAGE)
        {
            ...
            vla_offset = Nodecl::Conversion::make(
                    Nodecl::Add::make(
                        lhs.shallow_copy(),
                        Nodecl::Sizeof::make(
                            Nodecl::Type::make(destination_accessor._original_type),
                            Nodecl::NodeclBase::null(),
                            get_size_t_type()),
                        get_size_t_type()),
                    TL::Type::get_char_type().get_pointer_to());
        ...
        else if (IS_FORTRAN_LANGUAGE)
        {
            ...
            {
                std::stringstream shape_list;
                if (destination_accessor._original_type.no_ref().is_array())
                {
                    TL::Type array_type = destination_accessor._original_type.no_ref();
                    int dim = 1;
                    shape_list << "(";
                    while (array_type.is_array())
                    {
                        shape_list
                            << (dim > 1 ? ", " : "")
                            << "LBOUND(" << field_name << ", " << dim << ")"
                            << " : "
                            << "UBOUND(" << field_name << ", " << dim << ")"
                            ;
                ...
                TL::Source allocate_src;
                allocate_src << "ALLOCATE(" << as_symbol(destination_environment) << " % " << field_name << shape_list.str() << ")\n";
                allocate_stmt = allocate_src.parse_statement(context);
            }

            if (destination_accessor._environment_symbol.is_allocatable())
            {
                Nodecl::List actual_arguments = Nodecl::List::make(Nodecl::FortranActualArgument::make(destination_accessor._environment_symbol.make_nodecl(true)));
                TL::Symbol allocated = get_fortran_intrinsic_symbol<1>("allocated", actual_arguments, /* is_call */ 0);
